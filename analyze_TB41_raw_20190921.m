...
prevLchoice = prevchoice == 2;
prevRchoice = prevchoice == 1;
 

%% Make the predictor matrix
pred_types_cell = {'event', 'event', 'event', 'event', 'event', 'event', ...
    'whole-trial', 'whole-trial', 'continuous'};
groupings = {[1], [2,3], [4], [5,6], [7,8], [9]};
base_vars = {cue_onsetCells, left_onsetCells, right_onsetCells,...
     rewardsCell, left_choiceCells, right_choiceCells, prevLchoice, prevRchoice, balldata};
spline_filename = 'Splines/spline_basis_9_order5_30pts.mat';

[pred_allmat,pred_inds_cell,grouped_var_types] = make_predictor_matrix_generalcase(base_vars, pred_types_cell, spline_filename, groupings);
...
for i = 1:ntrialsgood
    T = trial_lengths(goodtrialsID(i));
    neural_act_mat{i} = dff(:, ixCue(goodtrialsID(i)) : ixCue(goodtrialsID(i) + 1) - 1)';
end

% Note: changed to dff from z-dff! 2019-09-22
%%

neural_matmat = cell2mat(neural_act_mat);
predall_matmat = cell2mat(pred_allmat);

save('TB41_encoding_structs_20190922.mat', 'neural_matmat', 'predall_matmat', 'neural_act_mat',...
...
xticks(1:size(relative_contrib, 2));
factors = {'Cue', 'Stim', 'Reward', 'Choice', 'PrevChoice', 'Movement'};
...
%% Plot 
figure;
scatter(relative_contrib(:, 2), relative_contrib(:, 4), 'o', 'MarkerFaceColor', 'b')
